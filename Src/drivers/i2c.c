...
static uint8_t urm09_slave_address = 0;
void i2c_init(void)
{
    ASSERT(!intitialized);
    ...
    RCC->APB1ENR1 |= 0x1 << 22;
    I2C2->CR1 &= ~0x1; // clears PE in CR1 reg to being intitialization
    I2C2->TIMINGR = 0x10D19CE4; // TIMINGR generated using stm32cubemx and default I2C speed
                                // settings i.e. standard mode
    I2C2->CR1 &= ~(0x1 << 17); // clear the bit to enable clock stretching
    I2C2->CR1 |= 0x1; // enables PE in CR1 reg to being intitialization

    intitialized = true;
}
void i2c_set_slave_addr(uint8_t addr)
{
    urm09_slave_address = addr;
}
static inline void i2c_set_start_condition(void)
{
    I2C2->CR2 |= (0x1 << 13); // set start condition
}
...
static void i2c_configure(uint8_t data_size, uint8_t addr_size, i2c_state_e state)
{
    ...
    I2C2->CR2 = 0;
    I2C2->CR2 |= urm09_slave_address << 1; // write the slave address
    uint16_t size;
    switch (state) {
    case I2C_SEND:
    ...
    I2C2->CR2 |= (0x1 << 25); // set AUTOEND mode
}
SUPPRESS_UNUSED
static i2c_result_code_e i2c_tx_flag_wait(void)
{
    ...
    while (!(I2C2->ISR & 0x1 << 1) && --retries) { };
    if (I2C2->ISR & 0x1 << 4) { // check for a NACK after transsimision
        I2C2->ICR |= 0x1 << 4; // clear NACKCF bit
...
static i2c_result_code_e i2c_start_tx_transfer(const uint8_t *urm09_memory_addr, uint8_t addr_size)
{
    ...
    I2C2->CR2 &= ~(0x1 << 10); // set to write direction
...
static i2c_result_code_e i2c_start_rx_transfer(const uint8_t *urm09_memory_addr, uint8_t addr_size)
{
    ...
    I2C2->ICR = I2C_ICR_STOPCF | I2C_ICR_NACKCF | I2C_ICR_BERRCF;
    I2C2->CR2 = 0;
    // I2C2->CR2 &= ~0x3FF; // clear the slave address
    I2C2->CR2 |= urm09_slave_address << 1; // write the slave address
...
{
    I2C2->CR2 |= (0x1 << 10); // set to read direction
    i2c_set_start_condition();
}
                           ...
                           uint8_t data_size)
{
    ...
    while (!(I2C2->ISR & (0x1 << 5)))
        ;
    I2C2->ICR |= 0x1 << 5; // clear the STOP detection flag in ISR
    return result;
};
